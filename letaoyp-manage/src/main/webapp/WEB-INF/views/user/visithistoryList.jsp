...
<div>
	<ul class="breadcrumb">
		<li>
			<a href="${ctx}/">Home</a> <span class="divider">/ user / 浏览历史</span>
		</li>
	</ul>
</div>
...
<div class="row-fluid sortable ui-sortable">	
	<div class="box span12">
		<div class="box-header well" data-original-title="">
			<form action="${ctx}/user/visithistory" method="post">
			<h2>
				<span class="ajax-link btn btn-primary" action="${ctx}/user/visithistory/create"><i class="icon-plus"></i> 浏览历史</span>
		...
		<table class="table table-bordered table-striped table-condensed">
			...
			<c:forEach items="${page.content}" var="item" varStatus="status">
				<tr <c:if test="${status.index % 2 == 0}">class="even"</c:if>
					...
					<td class="center ">
						<shiro:hasPermission name="visithistory:edit">
							<a class="btn btn-info ajax-link" style="padding: 0;"
								action="${ctx}/user/visithistory/update/${item.id}" title="编辑">
								<i class="icon-edit icon-white"></i> </a>
							<a class="btn btn-danger ajax-link" style="padding: 0;"
								action="${ctx}/user/visithistory/delete/${item.id}" title="删除">
