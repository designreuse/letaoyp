		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header with-border">
					<h3 class="box-title">
						<form id="searchForm" action="${ctx}/sku/brand">
							<button type="button" class="btn btn-default btn-sm checkbox-toggle" data-toggle-for="checkbox-toggled">
								<i class="fa fa-square-o"></i>
							</button>
							<div class="btn-group">
								<shiro:hasPermission name="sku:brand:edit">
									<a action="${ctx}/sku/brand/create" class="btn btn-default btn-sm" title="新增" data-toggle="modal"
									...
									<button type="button" class="btn btn-default btn-sm" title="失效" ajax data-data='{"active": "0"}'
										data-confirm="确认失效选中记录？" data-url="${ctx}/sku/brand/settings" data-method="post">
										<i class="fa fa-ban"></i>
									</button>
									<button type="button" class="btn btn-default btn-sm" title="激活" ajax data-data='{"active": "1"}'
										data-url="${ctx}/sku/brand/settings" data-method="post">
										<i class="fa fa-toggle-on"></i>
									</button>
								</shiro:hasPermission>

								<shiro:hasPermission name="sku:brand:delete">
									<button type="button" class="btn btn-default btn-sm" title="删除" ajax data-confirm="该操作将永久删除记录，确认删除？"
										data-url="${ctx}/sku/brand/delete" data-method="post">
						...
						<tbody id="checkbox-toggled">
							<c:forEach items="${page.content}" var="brand" varStatus="status">
								<tr>
									...
									<td class="center "><shiro:hasPermission name="sku:brand:edit">
				...
				<tags:pagination page="${page}" paginationSize="5" action="${ctx}/sku/brand" />
