		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header with-border">
					<h3 class="box-title">
						<form id="searchForm" action="${ctx}/sku/hotkey">
							<button type="button" class="btn btn-default btn-sm checkbox-toggle" data-toggle-for="checkbox-toggled">
								<i class="fa fa-square-o"></i>
							</button>
							<div class="btn-group">
								<shiro:hasPermission name="sku:hotkey:edit">
									<a action="${ctx}/sku/hotkey/create" class="btn btn-default btn-sm" title="新增" data-toggle="modal"
									...
									<button type="button" class="btn btn-default btn-sm" title="失效" ajax data-data='{"active": "0"}'
										data-confirm="确认失效选中记录？" data-url="${ctx}/sku/hotkey/settings" data-method="post">
										<i class="fa fa-ban"></i>
									</button>
									<button type="button" class="btn btn-default btn-sm" title="激活" ajax data-data='{"active": "1"}'
										data-url="${ctx}/sku/hotkey/settings" data-method="post">
										<i class="fa fa-toggle-on"></i>
									</button>
								</shiro:hasPermission>

								<shiro:hasPermission name="sku:hotkey:delete">
									<button type="button" class="btn btn-default btn-sm" title="删除" ajax data-confirm="该操作将永久删除记录，确认删除？"
										data-url="${ctx}/sku/hotkey/delete" data-method="post">
				...
				<div class="box-body">
					<table class="table table-bordered table-hover">
						<thead>
							<tr role="row">
								<th sortColumn="key">key</th>
								<th sortColumn="times">搜索次数</th>
						...
						<tbody id="checkbox-toggled">
							<c:forEach items="${page.content}" var="hotKey" varStatus="status">
								<tr>
									<td class="center"><input class="icheck" type="checkbox" value="${hotKey.id}">${hotKey.key}</td>
									<td class="center">${hotKey.times}</td>
									<td class="center">${hotKey.active}</td>
									<td class="center"><fmt:formatDate value='${hotKey.created}' pattern='yyyy-MM-dd HH:mm:ss' /></td>
									<td class="center"><fmt:formatDate value='${hotKey.modified}' pattern='yyyy-MM-dd HH:mm:ss' /></td>
									<td class="center "><shiro:hasPermission name="sku:hotkey:edit">
											<a href="#modal_form" action="${ctx}/sku/hotkey/update/${hotKey.id}" title="编辑" data-toggle="modal"
												data-target="#modal_form"><i class="fa fa-edit"></i> </a>
										</shiro:hasPermission> <shiro:hasPermission name="sku:hotkey:delete">
											<a href="${ctx}/sku/hotkey/delete/${hotKey.id}" single-delete title="删除"><i class="fa fa-trash-o"></i> </a>
				...
				<tags:pagination page="${page}" paginationSize="5" action="${ctx}/sku/hotkey" />
