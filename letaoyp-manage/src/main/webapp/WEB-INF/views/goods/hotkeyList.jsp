...
<div>
	<ul class="breadcrumb">
		<li>
			<a href="${ctx}/">Home</a> <span class="divider">/ goods / 热词</span>
		</li>
	</ul>
</div>
...
<div class="row-fluid sortable ui-sortable">	
	<div class="box span12">
		<div class="box-header well" data-original-title="">
			<form action="${ctx}/goods/hotkey" method="post">
			<h2>
				<span class="ajax-link btn btn-primary" action="${ctx}/goods/hotkey/create"><i class="icon-plus"></i> 热词</span>
		...
		<table class="table table-bordered table-striped table-condensed">
			...
			<c:forEach items="${page.content}" var="item" varStatus="status">
				<tr <c:if test="${status.index % 2 == 0}">class="even"</c:if>
					...
					<td class="center ">
						<shiro:hasPermission name="hotkey:edit">
							<a class="btn btn-info ajax-link" style="padding: 0;"
								action="${ctx}/goods/hotkey/update/${item.id}" title="编辑">
								<i class="icon-edit icon-white"></i> </a>
							<a class="btn btn-danger ajax-link" style="padding: 0;"
								action="${ctx}/goods/hotkey/delete/${item.id}" title="删除">
	   ...
	   <tags:pagination page="${page}" paginationSize="5" action="${ctx}/goods/hotkey" />
	</div>
	</div><!--/span-->
</div>
