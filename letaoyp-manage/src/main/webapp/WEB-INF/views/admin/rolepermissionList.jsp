		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header with-border">
					<h3 class="box-title">
						<form id="searchForm" action="${ctx}/admin/rolepermission">
							<button type="button" class="btn btn-default btn-sm checkbox-toggle" data-toggle-for="checkbox-toggled">
								<i class="fa fa-square-o"></i>
							</button>
							<div class="btn-group">
								<shiro:hasPermission name="admin:rolepermission:edit">
									<a action="${ctx}/admin/rolepermission/create" class="btn btn-default btn-sm" title="新增" data-toggle="modal"
									...
									<button type="button" class="btn btn-default btn-sm" title="失效" ajax data-data='{"active": "0"}'
										data-confirm="确认失效选中记录？" data-url="${ctx}/admin/rolepermission/settings" data-method="post">
										<i class="fa fa-ban"></i>
									</button>
									<button type="button" class="btn btn-default btn-sm" title="激活" ajax data-data='{"active": "1"}'
										data-url="${ctx}/admin/rolepermission/settings" data-method="post">
								...
								<shiro:hasPermission name="admin:rolepermission:delete">
									<button type="button" class="btn btn-default btn-sm" title="删除" ajax data-confirm="该操作将永久删除记录，确认删除？"
										data-url="${ctx}/admin/rolepermission/delete" data-method="post">
				...
				<div class="box-body">
					<table class="table table-bordered table-hover">
						...
						<tbody id="checkbox-toggled">
							<c:forEach items="${page.content}" var="rolePermission" varStatus="status">
								<tr>
									<td class="center"><input class="icheck" type="checkbox" value="${rolePermission.id}">
										${rolePermission.role.name}</td>
									<td class="center">${rolePermission.permission.description}</td>
									<td class="center">${rolePermission.active}</td>
									<td class="center"><fmt:formatDate value='${rolePermission.created}' pattern='yyyy-MM-dd HH:mm:ss' /></td>
									<td class="center"><fmt:formatDate value='${rolePermission.modified}' pattern='yyyy-MM-dd HH:mm:ss' /></td>
									<td class="center ">
									  <shiro:hasPermission name="admin:rolepermission:edit">
											<a href="#modal_form" action="${ctx}/admin/rolepermission/update/${rolePermission.id}"
				...
				<tags:pagination page="${page}" paginationSize="5" action="${ctx}/admin/rolepermission" />
