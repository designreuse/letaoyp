		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header with-border">
					<h3 class="box-title">
						<form id="searchForm" action="${ctx}/admin/log">
							<button type="button" class="btn btn-default btn-sm checkbox-toggle" data-toggle-for="checkbox-toggled">
								<i class="fa fa-square-o"></i>
							</button>
							<div class="btn-group">
								<shiro:hasPermission name="admin:log:edit">
									<a action="${ctx}/admin/log/create" class="btn btn-default btn-sm" title="新增" data-toggle="modal"
									...
									<button type="button" class="btn btn-default btn-sm" title="失效" ajax data-data='{"active": "0"}'
										data-confirm="确认失效选中记录？" data-url="${ctx}/admin/log/settings" data-method="post">
										<i class="fa fa-ban"></i>
									</button>
									<button type="button" class="btn btn-default btn-sm" title="激活" ajax data-data='{"active": "1"}'
										data-url="${ctx}/admin/log/settings" data-method="post">
								...
								<shiro:hasPermission name="admin:log:delete">
									<button type="button" class="btn btn-default btn-sm" title="删除" ajax data-confirm="该操作将永久删除记录，确认删除？"
										data-url="${ctx}/admin/log/delete" data-method="post">
				...
				<div class="box-body">
					<table class="table table-bordered table-hover">
						<thead>
							<tr role="row">
								<th sortColumn="type" >日志类型</th>
								<th sortColumn="content" >日志内容</th>
								<th sortColumn="remote_addr" >操作IP地址</th>
								<th sortColumn="user_agent" >用户代理</th>
								<th sortColumn="request_uri" >请求URI</th>
								<th sortColumn="method" >操作方式</th>
								<th sortColumn="params" >参数</th>
								<th sortColumn="exception" >异常信息</th>
								<th sortColumn="creator" >创建者</th>
						...
						<tbody id="checkbox-toggled">
							...
							<tr>
								<td class="center">
									<input class="icheck" type="checkbox" value="${log.id}">
${log.type}								</td>
								<td class="center">
${log.content}								</td>
								<td class="center">
...
${log.active}								</td>
								<td class="center">
									<fmt:formatDate value='${log.created}' pattern='yyyy-MM-dd HH:mm:ss'/>
								</td>
								<td class="center">
									<fmt:formatDate value='${log.modified}' pattern='yyyy-MM-dd HH:mm:ss'/>
								</td>
								<td class="center ">
									<shiro:hasPermission name="admin:log:edit">
										<a href="#modal_form" action="${ctx}/admin/log/update/${log.id}" title="编辑" data-toggle="modal"
				...
				<tags:pagination page="${page}" paginationSize="5" action="${ctx}/admin/log" />
