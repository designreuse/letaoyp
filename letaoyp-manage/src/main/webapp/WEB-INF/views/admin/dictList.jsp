		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header with-border">
					<h3 class="box-title">
						<form id="searchForm" action="${ctx}/admin/dict">
							...
							<div class="btn-group">
								<shiro:hasPermission name="admin:dict:edit">
									<a action="${ctx}/admin/dict/create" class="btn btn-default btn-sm" title="新增" data-toggle="modal"
									...
									<button type="button" class="btn btn-default btn-sm" title="失效" ajax data-data='{"active": "0"}'
										data-confirm="确认失效选中记录？" data-url="${ctx}/admin/dict/settings" data-method="post">
										<i class="fa fa-ban"></i>
									</button>
									<button type="button" class="btn btn-default btn-sm" title="激活" ajax data-data='{"active": "1"}'
										data-url="${ctx}/admin/dict/settings" data-method="post">
								...
								<shiro:hasPermission name="admin:dict:delete">
									<button type="button" class="btn btn-default btn-sm" title="删除" ajax data-confirm="该操作将永久删除记录，确认删除？"
										data-url="${ctx}/admin/dict/delete" data-method="post">
				...
				<div class="box-body">
					<table class="table table-bordered table-hover">
						<thead>
							<tr role="row">
								<th sortColumn="label" >标签名</th>
								<th sortColumn="value" >数据值</th>
								<th sortColumn="type" >类型</th>
								<th sortColumn="description" >描述</th>
								<th sortColumn="sort" >排序（升序）</th>
								<th sortColumn="remarks" >备注信息</th>
						...
						<tbody id="checkbox-toggled">
							<c:forEach items="${page.content}" var="dict" varStatus="status">
							<tr>
								<td class="center">
									<input class="icheck" type="checkbox" value="${dict.id}">
${dict.label}								</td>
								<td class="center">
${dict.value}								</td>
								<td class="center">
...
${dict.active}								</td>
								<td class="center">
									<fmt:formatDate value='${dict.created}' pattern='yyyy-MM-dd HH:mm:ss'/>
								</td>
								<td class="center">
									<fmt:formatDate value='${dict.modified}' pattern='yyyy-MM-dd HH:mm:ss'/>
								</td>
								<td class="center ">
									<shiro:hasPermission name="admin:dict:edit">
										<a href="#modal_form" action="${ctx}/admin/dict/update/${dict.id}" title="编辑" data-toggle="modal"
				...
				<tags:pagination page="${page}" paginationSize="5" action="${ctx}/admin/dict" />
