...
import com.iac.letaoyp.service.user.ShippingAddressService;


@Controller
@RequestMapping("user/shippingaddress")
public class ShippingAddressController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<ShippingAddress> page = shippingAddressService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "user/shippingaddressList";
	}
	
	@RequestMapping(value="/create", method = RequestMethod.GET)
	public String createForm(ModelMap model) {
		return "user/shippingaddressForm";
	}
	
	@RequestMapping(value="update/{id}", method = RequestMethod.GET)
	public String updateForm(ModelMap model, @PathVariable java.lang.Long id) {
		ShippingAddress shippingAddress = shippingAddressService.get(id);
	...
	@ResponseBody
	public AjaxResult update(@Valid @ModelAttribute("shippingAddress") ShippingAddress shippingAddress) {
	...
	public AjaxResult batchDelete(@RequestParam("ids") java.lang.Long[] ids) {
		...
		shippingAddressService.delete(ids);
		return AjaxResult.succeed();
	}
	...
	public AjaxResult settings(@RequestParam("ids")Long[] ids, @RequestParam("active")boolean active) {
		...
		shippingAddressService.updateActiveByIds(active, ids);
		return AjaxResult.succeed();
	}
	 ...
	 */
	@ModelAttribute
	public void getShippingAddress(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("shippingAddress", shippingAddressService.get(id));
		}
	}
}
