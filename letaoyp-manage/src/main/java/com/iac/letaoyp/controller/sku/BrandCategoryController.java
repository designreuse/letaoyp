...
@Controller
@RequestMapping("sku/brandcategory")
public class BrandCategoryController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<BrandCategory> page = brandCategoryService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "sku/brandcategoryList";
	}
	
	@RequestMapping(value="/create", method = RequestMethod.GET)
	public String createForm(ModelMap model) {
		return "sku/brandcategoryForm";
	}
	...
	@ResponseBody
	public AjaxResult update(@Valid @ModelAttribute("brandCategory") BrandCategory brandCategory) {
	...
	public AjaxResult batchDelete(@RequestParam("ids") java.lang.Long[] ids) {
		...
		brandCategoryService.delete(ids);
		return AjaxResult.succeed();
	}
	...
	public AjaxResult settings(@RequestParam("ids")Long[] ids, @RequestParam("active")boolean active) {
		...
		brandCategoryService.updateActiveByIds(active, ids);
		return AjaxResult.succeed();
	}
	 ...
	 */
	@ModelAttribute
	public void getBrandCategory(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("brandCategory", brandCategoryService.get(id));
		}
	}
}
