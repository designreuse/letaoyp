...
import com.iac.letaoyp.service.admin.RolePermissionService;
...
public class RolePermissionController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<RolePermission> page = rolePermissionService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "admin/rolepermissionList";
	}
	
	@RequestMapping(value="/create", method = RequestMethod.GET)
	public String createForm(ModelMap model) {
		return "admin/rolepermissionForm";
	}
	...
	@ResponseBody
	public AjaxResult update(@Valid @ModelAttribute("rolePermission") RolePermission rolePermission) {
	...
	public AjaxResult settings(@RequestParam("ids")Long[] ids, @RequestParam("active")boolean active) {
		...
		rolePermissionService.updateActiveByIds(active, ids);
		return AjaxResult.succeed();
	}
	 ...
	 */
	@ModelAttribute
	public void getRolePermission(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("rolePermission", rolePermissionService.get(id));
		}
	}
}
