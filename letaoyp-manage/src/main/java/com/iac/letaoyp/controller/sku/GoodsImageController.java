...
@Controller
@RequestMapping("sku/goodsimage")
public class GoodsImageController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<GoodsImage> page = goodsImageService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "sku/goodsimageList";
	}
	
	@RequestMapping(value="/create", method = RequestMethod.GET)
	public String createForm(ModelMap model) {
		return "sku/goodsimageForm";
	}
	
	@RequestMapping(value="update/{id}", method = RequestMethod.GET)
	public String updateForm(ModelMap model, @PathVariable java.lang.Long id) {
		GoodsImage goodsImage = goodsImageService.get(id);
	...
	@ResponseBody
	public AjaxResult update(@Valid @ModelAttribute("goodsImage") GoodsImage goodsImage) {
	...
	public AjaxResult settings(@RequestParam("ids")Long[] ids, @RequestParam("active")boolean active) {
		...
		goodsImageService.updateActiveByIds(active, ids);
		return AjaxResult.succeed();
	}
	 ...
	 */
	@ModelAttribute
	public void getGoodsImage(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("goodsImage", goodsImageService.get(id));
		}
	}
}
