...
public class CartController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<Cart> page = cartService.findPage(searchParams, pageNumber, pageSize, sortType);
	...
	@ResponseBody
	public AjaxResult update(@Valid @ModelAttribute("cart") Cart cart) {
	...
	public AjaxResult settings(@RequestParam("ids")Long[] ids, @RequestParam("active")boolean active) {
		...
		cartService.updateActiveByIds(active, ids);
		return AjaxResult.succeed();
	}
	 ...
	 */
	@ModelAttribute
	public void getCart(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
