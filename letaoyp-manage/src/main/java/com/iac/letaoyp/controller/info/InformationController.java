...
import com.iac.letaoyp.service.info.InformationService;

@Controller
@RequestMapping("info/information")
public class InformationController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<Information> page = informationService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "info/informationList";
	}
	
	/** 进入新增 */
	@RequestMapping(value="/create", method = RequestMethod.GET)
	public String createForm(ModelMap model) {
		return "info/informationForm";
	}
	...
	public String update(@Valid @ModelAttribute("information") Information information, 
			RedirectAttributes redirectAttributes, BindingResult errors, ModelMap model) {
		informationService.save(information);
		redirectAttributes.addFlashAttribute("message", "更新成功");
		return "redirect:/info/information";
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") java.lang.Long[] items,
		...
		for(int i = 0; i < items.length; i++) {
			informationService.delete(items[i]);
		}
		redirectAttributes.addFlashAttribute("message", "删除成功");
		return "redirect:/info/information";
	}
	 ...
	 */
	@ModelAttribute
	public void getInformation(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("information", informationService.get(id));
		}
	}
}
