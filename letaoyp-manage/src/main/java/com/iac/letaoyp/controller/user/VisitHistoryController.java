...
import com.iac.letaoyp.service.user.VisitHistoryService;


@Controller
@RequestMapping("user/visithistory")
public class VisitHistoryController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<VisitHistory> page = visitHistoryService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "user/visithistoryList";
	}
	
	@RequestMapping(value="/create", method = RequestMethod.GET)
	public String createForm(ModelMap model) {
		return "user/visithistoryForm";
	}
	
	@RequestMapping(value="update/{id}", method = RequestMethod.GET)
	public String updateForm(ModelMap model, @PathVariable java.lang.Long id) {
		VisitHistory visitHistory = visitHistoryService.get(id);
	...
	@ResponseBody
	public AjaxResult update(@Valid @ModelAttribute("visitHistory") VisitHistory visitHistory) {
	...
	public AjaxResult batchDelete(@RequestParam("ids") java.lang.Long[] ids) {
		...
		visitHistoryService.delete(ids);
		return AjaxResult.succeed();
	}
	...
	public AjaxResult settings(@RequestParam("ids")Long[] ids, @RequestParam("active")boolean active) {
		...
		visitHistoryService.updateActiveByIds(active, ids);
		return AjaxResult.succeed();
	}
	 ...
	 */
	@ModelAttribute
	public void getVisitHistory(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("visitHistory", visitHistoryService.get(id));
		}
	}
}
