...
public class OrderItemController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<OrderItem> page = orderItemService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "user/orderitemList";
	}
	...
	public String update(@Valid @ModelAttribute("orderItem") OrderItem orderItem, 
			RedirectAttributes redirectAttributes, BindingResult errors, ModelMap model) {
		orderItemService.save(orderItem);
		redirectAttributes.addFlashAttribute("message", "更新成功");
		return "redirect:/user/orderitem";
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") java.lang.Long[] items,
		...
		for(int i = 0; i < items.length; i++) {
			orderItemService.delete(items[i]);
		}
		redirectAttributes.addFlashAttribute("message", "删除成功");
		return "redirect:/user/orderitem";
	}
	 ...
	 */
	@ModelAttribute
	public void getOrderItem(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("orderItem", orderItemService.get(id));
		}
	}
}
