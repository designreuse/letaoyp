...
function file_upload() {
    ...
    $form.ajaxSubmit({
        ...
        success: function(data) {
            if (data.success) {
            	if(config.success_func && typeof window[config.success_func] == 'function')
...
function picUpload() {
    ...
    $form.ajaxSubmit({
        ...
        success: function(d) {
            if (d.success) {
                $inputImg.attr('src', config.url_prepend ? config.url_prepend + d.pic : d.pic);
                $inputId.val(config.url_prepend ? config.url_prepend + d.pic : d.pic);
            } else {
...
function datepicker() {
    ...
    var defaultOption = {
        ...
        isShowClear: 'true',
        onpicked: function() { $this.blur(); } // 选择日期后让datapicker失去焦点以触发onchange事件
    };
